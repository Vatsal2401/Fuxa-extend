# TracIoT — Industrial Dashboard Builder
...
TracIoT is a full **rebrand + UI/UX overhaul** of FUXA — same underlying SCADA / HMI engine, completely redesigned editor experience. Key additions:
...
- 🌐 **13 i18n languages** rebranded with `TracIoT` text + "Powered by FUXA · frangoteam" attribution in the About dialog
...
```bash
git clone https://github.com/Vatsal2401/Fuxa-extend.git tracIoT
cd tracIoT
...
```bash
docker build -t tracIoT:local .
docker run -d --name tracIoT -p 1881:1881 \
...
gcloud compute instances create tracIoT-fuxa \
  --zone=asia-south1-a \
...
# 3. Open the firewall
gcloud compute firewall-rules create allow-tracIoT-1881 \
  --direction=INGRESS --action=ALLOW \
  --rules=tcp:1881 --source-ranges=0.0.0.0/0 \
  --target-tags=tracIoT-fuxa

# 4. SSH in, install Node + clone + start
gcloud compute ssh tracIoT-fuxa --zone=asia-south1-a --command='
  curl -fsSL https://deb.nodesource.com/setup_18.x | sudo bash -
  sudo apt-get install -y nodejs git build-essential python3 libsqlite3-dev
  sudo npm install -g pm2

  git clone -b feat/design-system https://github.com/Vatsal2401/Fuxa-extend.git ~/tracIoT
  cd ~/tracIoT/server && npm install
  pm2 start main.js --name tracIoT
  pm2 save
  pm2 startup systemd -u $USER --hp $HOME
'

# 5. Get the public IP
gcloud compute instances describe tracIoT-fuxa --zone=asia-south1-a \
...
```bash
gcloud compute instances stop  tracIoT-fuxa --zone=asia-south1-a
gcloud compute instances start tracIoT-fuxa --zone=asia-south1-a
```
...
Every visible "FUXA" string was replaced with **`TracIoT`** across:
...
To swap the placeholder `TracIoT` for your actual brand name:

```bash
find client/src -type f \( -name '*.json' -o -name '*.html' \) \
     -exec sed -i 's/TracIoT/YourBrandName/g' {} +
